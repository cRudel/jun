//note: I could only get this to work in eclipse because you
// I needed the connector/j jar and I knew how to load it into eclipse
// trying to do this in terminal wasn't working
import java.sql.*;
           ...
           finally
           {
               if (conn != null)
               {
                   try
                   {
                	   ...
                	   rs = s.getResultSet();
                	   while(rs.next()) {
                		   System.out.println("Licenses expiring before 12-21-2022 driver_name: " + rs.getString("driver_name"));;
                	   }
                       rs = s.getResultSet();
                       s.executeQuery("SELECT DISTINCT driver_name FROM driver d, exam e1, exam e2 WHERE d.driver_ssn = e1.driver_ssn AND d.driver_ssn = e2.driver_ssn AND e1.exam_type = e2.exam_type AND e1.branch_id = e2.branch_id AND e1.exam_score != e2.exam_score");
                       rs = s.getResultSet();
                	   while(rs.next()) {
                		   System.out.println("Drivers who took 2 exams driver_name: " + rs.getString("driver_name"));;
                	   }
                	   s.executeQuery("SELECT DISTINCT driver_name FROM driver WHERE driver_name NOT IN (SELECT driver_name FROM exam AS a JOIN exam AS b ON a.driver_ssn = b.driver_ssn AND a.exam_date > b.exam_date AND a.exam_score > b.exam_score, driver d WHERE a.driver_ssn = d.driver_ssn)");
                	   rs = s.getResultSet();
                	   while(rs.next()) {
